...
class hsResNet(nn.Module):
    def __init__(self, depth, pretrained=True, cut_at_pooling=False,
        ...
        from .hs_resnet import hs_resnet50
        hsresnet = hs_resnet50()

        '''
        print("Loading the supervised HSresnet50 pre-trained model on eBay")
        pretrained_net_dict = torch.load('hsresnet50_supervised_pretrain.pth.tar', map_location='cpu')
        new_state_dict = OrderedDict()
        for k, v in pretrained_net_dict["state_dict"].items():
            name = k[7:] # remove `module.`
            new_state_dict[name] = v
        hsresnet.load_state_dict(new_state_dict, strict = True)
        '''
        
        
        hsresnet.layer4[0].conv2.stride = (1,1)
        hsresnet.layer4[0].downsample[0].stride = (1,1)
        gap = GeneralizedMeanPoolingP() #nn.AdaptiveAvgPool2d(1)
        print("The init norm is ",gap)
        waveblock = Waveblock()
        
        self.base = nn.Sequential(
            hsresnet.conv1, hsresnet.bn1, hsresnet.relu,
            hsresnet.layer1,
            hsresnet.layer2, waveblock,
            hsresnet.layer3, waveblock,
            hsresnet.layer4, gap
        ).cuda()
        ...
        if not self.cut_at_pooling:
            ...
            out_planes = hsresnet.fc.in_features
...
def hsresnet50(**kwargs):
    return hsResNet(50, **kwargs)
