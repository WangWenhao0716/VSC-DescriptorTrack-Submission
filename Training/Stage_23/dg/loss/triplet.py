...
def _batch_hard_n(mat_distance, mat_similarity, top_n, indice=False):
    sorted_mat_distance, positive_indices = torch.sort(mat_distance + (-9999999.) * (1 - mat_similarity), dim=1, descending=True)
    hard_p = sorted_mat_distance[:, 0]
    hard_p_indice = positive_indices[:, 0]
    sorted_mat_distance, negative_indices = torch.sort(mat_distance + (9999999.) * (mat_similarity), dim=1, descending=False)
    hard_n = sorted_mat_distance[:, top_n]
    hard_n_indice = negative_indices[:, top_n]
    if(indice):
        return hard_p, hard_n, hard_p_indice, hard_n_indice
    return hard_p, hard_n
...
class SoftTripletLoss_softhard(nn.Module):

    def __init__(self, margin=None, normalize_feature=False):
        super(SoftTripletLoss_softhard, self).__init__()
        self.margin = margin
        self.normalize_feature = normalize_feature
    ...
    def forward(self, emb1, emb2, label):
        ...
        dist_ap, dist_an, ap_idx, an_idx = _batch_hard_n(mat_dist, mat_sim, top_n=1, indice=True)
        assert dist_an.size(0)==dist_ap.size(0)
        triple_dist = torch.stack((dist_ap, dist_an), dim=1)
        triple_dist = F.log_softmax(triple_dist, dim=1)
        if (self.margin is not None):
