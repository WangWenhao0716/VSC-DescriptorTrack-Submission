...
from dg.trainers_cos_ema_tune_gt_ng_cls import Trainer
...
def main_worker(args):
    ...
    dataset_source_small, num_classes_small, train_loader_small, _ = \
        get_data(args.dataset_source_small, args.data_dir, args.height, args.width, \
             args.batch_size, args.workers, args.num_instances, iters)
    ...
    ppp_T = '/raid/VSC/data/pull_gt_all_shuffle/gt_query/'
    ...
    ppp_O = '/raid/VSC/data/pull_gt_all_shuffle/gt_ref/'
    ...
    model = models_gem_waveblock_balance_cos.create(
        args.arch, num_features=args.features, dropout=args.dropout, num_classes=num_classes, num_classes_small=num_classes_small
    )
    model.cuda()
    model = nn.DataParallel(model)
    print("Loading the support model!!!")
    model_support = models_gem_waveblock_balance_cos.create(
        'cotnet50', num_features=args.features, dropout=args.dropout, num_classes=10_0000
    )
    model_support.cuda()
    model_support = nn.DataParallel(model_support)
    path_support = './train_v1_50C_s27_512_twolosses_m0.6_all_bw.pth.tar'
    ...
    with open("pull_gt_all_shuffle_q.pickle", "rb") as ff:
        my_dict_q = pickle.load(ff)
    
    with open("pull_gt_all_shuffle_r.pickle", "rb") as ff:
        my_dict_r = pickle.load(ff)
    
    # Trainer
    trainer = Trainer(model, model_support, num_classes, num_classes_small, margin=args.margin)
    
    # Start training
    for epoch in range(start_epoch, args.epochs):
        train_loader.new_epoch()
        train_loader_small.new_epoch()
        trainer.train(epoch, train_loader, train_loader_small, support_loader, support_loader_T, support_loader_O, idx_to_class_T, my_dict_q, my_dict_r, optimizer, ema, train_iters=len(train_loader), print_freq=args.print_freq) 
        lr_scheduler.step()
        is_best = False
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ds-small', '--dataset-source-small', type=str, default='randperson',
                        choices=datasets.names())
