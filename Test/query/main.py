...
def videos_to_images_half(query_subset_video_ids):
    path_save = '/dev/shm/query_half_second_ff_v2/'
...
def extract_feature_50SK_half():
    ...
    os.system('conda run --no-capture-output -n condaenv python extract_feature.py \
      --image_dir /dev/shm/query_half_second_ff_v2 \
...
def main():
    ...
    videos_to_images_half(query_subset_video_ids)
    ...
    print("Transfer videos to images every one second using imageio")
    videos_to_images(query_subset_video_ids_filter)
